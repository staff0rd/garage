...
export default function BuyScreen() {
  ...
  const offerRefresh = useSelector(
    (state: RootState) => state.buyScreen.offerRefresh
  );
  const money = useSelector((state: RootState) => state.app.money);
  const { deliveryMinSeconds, deliveryMaxSeconds } = useSelector(
  ...
  const confirm = (offer: BuyableItem) => {
    setPromptOptions({
      ...
      ok: () => {
        ...
        arrive.setSeconds(
          arrive.getSeconds() +
            Random.between(deliveryMinSeconds, deliveryMaxSeconds)
        );
        dispatch(
          addOrder({
      ...
      <StandardDialog open={open} handleClose={handleClose}>
        <div className={classes.table}>
          <StandardTable
            ...
            title={
              <div>
                Buy (<Countdown date={offerRefresh} />)
              </div>
            }
          />
        </div>
